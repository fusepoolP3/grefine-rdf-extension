...
RdfPrefixesManager.prototype._showPrefixes = function () {
    ...
    // import neologism repository
    $('<a href="#" class ="add-neologism-box">import neologism repository</a>').bind('click', function (e) {
        e.preventDefault();
        self._addNeologism();
    }).appendTo(self._dialog._rdf_schema_prefixes);
...
RdfPrefixesManager.prototype._addNeologism = function (msg, def_prefix, onDone) {
    var self = this;
    var widget = new ImportNeologismWidget(self);

    widget.show(msg, def_prefix, function (prefixes) {
        for (var i = 0; i < prefixes.length; i++) {
            var prefix = prefixes[i];
            self._prefixes.push({name: prefix.prefix, uri: prefix.uri});
        }
        self._showPrefixes();
        onDone();
    });
